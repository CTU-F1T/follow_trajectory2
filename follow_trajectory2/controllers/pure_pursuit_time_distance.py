...
PARAMETERS = [
    ...
    ("k_const", {
        ...
        "description": "Lookahead time-distance [s]"
    }),

    # Time-based lookahead
    ("k_speed", {
        "default": 0.07, "min": -1.2, "max": 1.2,
        "description": "Lookahead time-distance for speed [s]"
    }),

    # Limits for the lookahead distance
    ("min_lookfwd_dist", {
        "default": 0.3, "min": 0.0, "max": 5.0,
        "description": "Minimum lookahead distance [m]", "ns": "default"
    }),
    ("max_lookfwd_dist", {
        "default": 1.1, "min": 0.0, "max": 5.0,
        "description": "Maximum lookahead distance [m]", "ns": "default"
    }),

    # Experimental correction
    ("k_corr", {
        "default": -0.06, "min": -0.2, "max": 0.2,
        "description": "Steer correction per curvature"
    }),
    ("k_corr2", {
        "default": 0.0, "min": -0.5, "max": 0.5,
        "description": "LA steer correction per curvature"
    }),
    ("v_thresh", {
        "default": 3.7, "min": 0.5, "max": 7.0,
        "description": "Speed threshold for using different limits"
    }),
    ("max_lookfwd_dist2", {
        "default": 1.4, "min": 0.0, "max": 5.0,
        "description": "Maximum LA [m]"
    }),
    ("k_accel", {
        "default": 0.0, "min": -1.0, "max": 1.0,
        "description": "Coefficient to correct speed"
    }),
    ("k_time", {
        "default": 3.5, "min": 0.0, "max": 10.0,
        "description": "Coefficient to correct speed from time difference"
    }),
    ("time_tracking", False),
    ("k_ori", {
        "default": 0.05, "min": -1.0, "max": 1.0,
        "description": "Coefficient to correct orientation from difference."
    }),
    ("k_ori2", {
        "default": -0.001, "min": -1.0, "max": 1.0,
        "description": "LA Coefficient to correct orientation from difference."
    }),
    ("time_interpolation", False),
    ("dt", {
        "default": 0.02, "min": 0.001, "max": 1.0,
        "description": "Delta t for reinterpolating the trajectory to make "
                       "the points isochronal."
    }),
    ("k_alpha", {
        "default": 1.0, "min": 0.0, "max": 2.0,
        "description": "Weight for alpha steering"
    }),
    ("k_delta", {
        "default": 0.0, "min": -1.0, "max": 1.0,
        "description": "Weight for delta difference"
    }),
    ("k_delta2", {
        "default": 0.0, "min": -1.0, "max": 1.0,
        "description": "LA Weight for delta difference"
    }),
]
...
class Controller(ControllerABC):
    """Implementation of the Pure Pursuit controller with time shift."""
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.P2.reconfigure(
            namespace = "pure_pursuit_time_dist", node = self.node
        )
    ...
    def compute(self, controller, tpoint, trajectory, trajectory_i):
        ...
        if tpoint.t is None:
            raise ValueError(
                "Unable to do the time lookahead when the trajectory "
                "lacks timeshifts."
            )
        ...
        act_velocity = (
            desired_velocity
            + acc * self.Vehicle.dt * self.P2.k_accel
            + t_dist * self.P2.k_time
        )
        ...
        for i in range(trajectory.length):
            time_distance = trajectory.time_distance(
                trajectory_i, trajectory_i + i
            )
            look_ahead_dist = point_distance(
                self.Vehicle, trajectory.get(trajectory_i + i)
            )

            # Note: We make k_const as a time distance parameter.
            # It does not make any sense to use speed in here, as the PP
            # approach computes:
            # P * v = s ~ meaning that P ~ time.
            if (
                (
                    time_distance > self.P2.k_const
                    and look_ahead_dist > self.P2.min_lookfwd_dist
                ) or (
                    look_ahead_dist > (
                        self.P2.max_lookfwd_dist2
                        if act_velocity > self.P2.v_thresh
                        else self.P2.max_lookfwd_dist
                    )
                )
            ):
                # Note: This is now constrained by min/max la distance.
                # Sidenote: The limits are not hard, i.e., next point after
                #           breaking the limit is returned. However,
        ...
        steer_angle = (
            self.P2.k_alpha * math.atan(
                (2 * self.Vehicle.L * math.sin(alpha)) / look_ahead_dist
            )
            + self.P2.k_corr * trajectory.get(trajectory_i).k
            + self.P2.k_corr2 * trajectory.get(goal_point_id).k
            + self.P2.k_ori * math.atan2(
                math.sin(trajectory.get(trajectory_i).yaw - self.Vehicle.yaw),
                math.cos(trajectory.get(trajectory_i).yaw - self.Vehicle.yaw)
            )
            + self.P2.k_ori2 * math.atan2(
                math.sin(trajectory.get(goal_point_id).yaw - self.Vehicle.yaw),
                math.cos(trajectory.get(goal_point_id).yaw - self.Vehicle.yaw)
            )
            + self.P2.k_delta * trajectory.get(trajectory_i).delta
            + self.P2.k_delta2 * trajectory.get(goal_point_id).delta
        )
